...
export const RECEIPT_CONFIGS = {
    traveler: {
        ...
        fields: (order, admin_data, customer, details, formatDate) => [
            ...
            { id: 'batch_number', value: admin_data?.batch_number, x: 82.8, y: 42.4, bold: true, width: '4%' },
            { id: 'batch_date', value: admin_data?.batch_date, x: 63.8, y: 42.4, bold: true, width: '10%' },
            { id: 'order_number', value: order.order_id, x: 46.1, y: 42.4, bold: true, width: '11%' },
            { id: 'order_date', value: formatDate(order.created_at), x: 20.5, y: 42.4, bold: true, width: '10%' },
            { id: 'full_name', value: customer?.full_name, x: 81.3, y: 43.7, bold: true, width: '20%' },
            { id: 'mother_name', value: admin_data?.mother_name, x: 39.1, y: 43.7, bold: true, width: '20%' },
            { id: 'phone', value: customer?.phone, x: 30.2, y: 45.5, bold: true, width: '15%' },
            { id: 'ticket_number', value: admin_data?.ticket_number || details?.ticket_number, x: 82.2, y: 47.3, bold: true, width: '10%' },
            { id: 'travel_date', value: details?.travelDate, x: 54.3, y: 47.3, bold: true, width: '15%' },
            { id: 'travel_time', value: admin_data?.travel_time, x: 32.1, y: 47.2, bold: true, width: '10%' },
            { id: 'passport_number', value: admin_data?.passport_number, x: 83.4, y: 48.9, bold: true, width: '15%' },
            { id: 'issue_date', value: admin_data?.passport_issue_date, x: 53.5, y: 48.9, bold: true, width: '15%' },
            { id: 'expiry_date', value: admin_data?.passport_expiry_date, x: 32.6, y: 48.9, bold: true, width: '15%' },
            { id: 'destination', value: details?.destination, x: 82.2, y: 50.6, bold: true, width: '20%' },
            { id: 'travel_type', value: details?.travelType === 'air' ? 'جوي' : (details?.travelType === 'land' ? 'بري' : 'بحري'), x: 33.6, y: 50.6, bold: true, width: '10%' },
            { id: 'outlet_name', value: details?.pickupLocationName || admin_data?.travel_agency || 'وكيل المطار', x: 81.5, y: 52.2, bold: true, width: '20%' },
            { id: 'amount_usd', value: parseFloat(details?.usdAmount || details?.amountUSD || 0).toLocaleString(), x: 86, y: 53.8, bold: true, width: '10%' },
            { id: 'selling_rate', value: Math.round(parseFloat(details?.iqdAmount || 0) / parseFloat(details?.usdAmount || 1)).toLocaleString(), x: 83.9, y: 55.4, bold: true, width: '9%' },
            { id: 'commission', value: Math.round(parseFloat(details?.iqdAmount || 0) * 0.02).toLocaleString(), x: 71, y: 55.4, bold: true, width: '7%' },
            { id: 'total_iqd_final', value: parseFloat(details?.iqdAmount || 0).toLocaleString(), x: 32.8, y: 55.4, bold: true, width: '10%' },
        ],
        stamps: {
            airport: { x: 67.9, y: 67.4, width: 300, height: 300, rotate: -12 },
            signature: { x: 22.2, y: 58.5, width: 350, height: 150, rotate: 0 },
            company: { x: 36.1, y: 67.6, width: 300, height: 300, rotate: 50 }
        },
        qr: { x: 13, y: 68.9, size: 80 }
    },
    western_union: {
        template: '/assets/receipts/western-union.jpg',
        fields: (order, admin_data, customer, details, formatDate) => [
            { id: 'order_id', value: details?.mtcn || order.order_id?.replace(/^WU-/, ''), x: 90, y: 39.2, bold: true, fontSize: 8 },
            { id: 'date', value: formatDate(order.created_at), x: 90, y: 40, bold: true, fontSize: 8 },
            { id: 'sender', value: details?.senderName, x: 34.9, y: 39.6, bold: true, fontSize: 8 },
            { id: 'sender_address', value: details?.senderAddress, x: 34.9, y: 42.5, bold: true, fontSize: 8 },
            { id: 'sender_phone', value: details?.senderPhone, x: 34.9, y: 55.1, bold: true, fontSize: 8 },
            { id: 'receiver', value: details?.receiverName, x: 60.1, y: 39.5, bold: true, fontSize: 8 },
            { id: 'receiver_address', value: details?.receiverAddress, x: 60.1, y: 54, bold: true, fontSize: 8 },
            { id: 'receiver_phone', value: details?.receiverPhone, x: 60.1, y: 56, bold: true, fontSize: 8 },
            { id: 'receiver_country', value: getCountryTranslation(details?.receiverCountry), x: 60.1, y: 47.7, bold: true, fontSize: 8 },
            { id: 'receiver_country_2', value: getCountryTranslation(details?.receiverCountry), x: 90, y: 42.4, bold: true, fontSize: 8 },
            { id: 'id_type', value: details?.idType === 'passport' ? 'جواز سفر' : details?.idType === 'national_id' ? 'بطاقة هوية' : details?.idType, x: 34.9, y: 43.5, bold: true, fontSize: 8 },
            { id: 'id_number', value: admin_data?.id_number, x: 34.9, y: 48.9, bold: true, fontSize: 8 },
            { id: 'id_issue_date', value: admin_data?.id_issue_date, x: 34.9, y: 45.6, bold: true, fontSize: 8 },
            { id: 'id_issuing_authority', value: admin_data?.id_issuing_authority, x: 34.9, y: 46.6, bold: true, fontSize: 8 },
            { id: 'id_expiry_date', value: admin_data?.id_expiry_date, x: 34.9, y: 47.7, bold: true, fontSize: 8 },
            { id: 'purpose', value: details?.purpose === 'trade' ? 'تجارة' : details?.purpose === 'family_expenses' ? 'نفقات الأسرة' : details?.purpose === 'medical' ? 'علاج' : (details?.purpose || ''), x: 90, y: 55.1, bold: true, fontSize: 8 },
            { id: 'amount', value: `${details?.amount} ${details?.currency}`, x: 90, y: 52.8, bold: true, fontSize: 8 },
            { id: 'currency_name', value: details?.currency, x: 90, y: 54, bold: true, fontSize: 8 },
            { id: 'usd_amount', value: details?.usdAmount ? `${parseFloat(details.usdAmount).toFixed(2)} USD` : '', x: 90, y: 44.4, bold: true, fontSize: 8 },
            { id: 'exchange_rate', value: details?.exchangeRateUSD ? parseFloat(details.exchangeRateUSD).toFixed(4) : '', x: 90, y: 51.7, bold: true, fontSize: 8 },
            { id: 'fee', value: `${details?.serviceFee?.toLocaleString()} IQD`, x: 90, y: 45.5, bold: true, fontSize: 8 },
        ],
        stamps: {
            company: { x: 83.3, y: 63.7, width: 250, height: 250, rotate: 0 }
        },
    },
    moneygram: {
