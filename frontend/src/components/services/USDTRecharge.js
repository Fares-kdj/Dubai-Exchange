...
const USDTRecharge = () => {
  ...
  const networks = [
    ...
    { 
      ...
      icon: '🟣',
      description: currentLanguage === 'ar' ? 'رسوم منخفضة' : 'Low Fees',
      fee: '0.1 USDT',
    ...
    { 
      ...
      labelEn: 'Solana (SOL)', 
      icon: '🌈',
      description: currentLanguage === 'ar' ? 'سريعة جداً' : 'Very Fast',
      fee: '0.01 USDT',
      color: 'from-gradient-500 to-teal-500'
    },
    { 
      value: 'arbitrum', 
      labelAr: 'Arbitrum', 
      ...
      icon: '🔵',
      description: currentLanguage === 'ar' ? 'Layer 2 إيثريوم' : 'Ethereum Layer 2',
      fee: '0.5 USDT',
  ...
  const validateForm = () => {
    ...
    if (!formData.network) newErrors.network = currentLanguage === 'ar' ? 'اختر الشبكة' : 'Select network';
    if (!formData.walletAddress.trim()) {
      newErrors.walletAddress = currentLanguage === 'ar' ? 'عنوان المحفظة مطلوب' : 'Wallet address required';
    } else if (!validateWalletAddress(formData.walletAddress, formData.network)) {
      newErrors.walletAddress = currentLanguage === 'ar' ? 'عنوان غير صالح لهذه الشبكة' : 'Invalid address for this network';
    }
    if (!formData.amount || parseFloat(formData.amount) < 10) {
      newErrors.amount = currentLanguage === 'ar' ? 'الحد الأدنى 10 USDT' : 'Minimum 10 USDT';
    }
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
            ...
            <h1 className={`text-3xl md:text-4xl font-bold mb-3 ${isDark ? 'text-white' : 'text-slate-900'}`}>
              {currentLanguage === 'ar' ? 'شحن USDT' : 'USDT Recharge'}
            </h1>
            <p className={isDark ? 'text-slate-400' : 'text-slate-600'}>
              {currentLanguage === 'ar' ? 'شحن محفظتك بالعملات المشفرة' : 'Recharge your crypto wallet'}
            </p>
          </motion.div>
          ...
          >
            ...
            <div className={`rounded-3xl border-2 shadow-xl p-8 mb-6 ${
              ...
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>
                  {currentLanguage === 'ar' ? 'اختر الشبكة' : 'Select Network'}
                </h2>
              </div>
                  ...
                  >
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className={`font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>
                        {currentLanguage === 'ar' ? network.labelAr : network.labelEn}
                      </span>
                    </div>
                    <p className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{network.description}</p>
                    <p className="text-xs text-amber-500 mt-1">{currentLanguage === 'ar' ? 'الرسوم:' : 'Fee:'} {network.fee}</p>
                  </motion.button>
                ))}
              </div>
              ...
              <div className="space-y-2">
                <Label className={isDark ? 'text-slate-300' : 'text-slate-700'}>
                  ...
                  {currentLanguage === 'ar' ? 'عنوان المحفظة' : 'Wallet Address'} *
                </Label>
                <div className="relative">
                  <Input
                    value={formData.walletAddress}
                    onChange={(e) => handleInputChange('walletAddress', e.target.value)}
                    className={`h-12 font-mono text-sm ${isDark ? 'bg-slate-700 border-slate-600 text-white' : 'border-slate-300'}`}
                    placeholder={selectedNetwork ? 
                      (formData.network === 'trc20' ? 'TXyz...' : '0x...') : 
                      (currentLanguage === 'ar' ? 'اختر الشبكة أولاً' : 'Select network first')
                    }
                    dir="ltr"
            ...
            <div className={`rounded-3xl border-2 shadow-xl p-8 mb-6 ${
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <Label className={isDark ? 'text-slate-300' : 'text-slate-700'}>
                    {currentLanguage === 'ar' ? 'المبلغ (USDT)' : 'Amount (USDT)'} *
                  </Label>
                  <Input
                    type="number"
                ...
                >
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between">
                      <span className={isDark ? 'text-slate-400' : 'text-slate-600'}>{currentLanguage === 'ar' ? 'المبلغ USDT' : 'USDT Amount'}</span>
                      <span className={`font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{formData.amount} USDT</span>
                    </div>
                    <div className="flex justify-between">
                      <span className={isDark ? 'text-slate-400' : 'text-slate-600'}>{currentLanguage === 'ar' ? 'سعر الصرف' : 'Exchange Rate'}</span>
                      <span className={`font-semibold ${isDark ? 'text-white' : ''}`}>1 USDT = {usdtToIQD.toLocaleString()} IQD</span>
                    </div>
                    <div className="flex justify-between">
                      <span className={isDark ? 'text-slate-400' : 'text-slate-600'}>{currentLanguage === 'ar' ? 'المقابل بالدينار' : 'Amount in IQD'}</span>
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold text-amber-500">{calculateFee().toLocaleString()} IQD</span>
                    </div>
                    <div className={`border-t pt-3 mt-3 ${isDark ? 'border-teal-700/50' : 'border-teal-200'}`}>
                      <div className="flex justify-between items-center">
                        <span className={`font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{currentLanguage === 'ar' ? 'الإجمالي للدفع' : 'Total to Pay'}</span>
                        <span className="font-bold text-2xl text-teal-500">{calculateTotal().toLocaleString()} IQD</span>
                      </div>
                    </div>
                  </div>
                </motion.div>
            ...
            >
              {loading ? (
                ...
                <>
                  ...
                  {currentLanguage === 'ar' ? 'تأكيد طلب الشحن' : 'Confirm Recharge Request'}
                </>
              )}
            </motion.button>
          </motion.form>
